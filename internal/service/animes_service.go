...
type AnimesRepository interface {
	...
	UpdateUnique(anime models.Animes, id string) error
}
...
func (s *AnimeService) UpdateAnimeForId(anime models.Animes, id string) error {
	if id == "" {
		return errors.New("ID inválido")
	}

	return s.repo.UpdateUnique(anime, id)
}
